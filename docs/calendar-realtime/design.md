...
カレンダー上の日付、今日判定、月移動、イベントのグルーピング、公開 API の日付範囲はすべて `Asia/Tokyo` 基準で扱います。Vercel などのサーバー実行環境や閲覧ブラウザのローカルタイムゾーンには依存させません。
...
| `rangeDays` | `28` | 最大 `60` |
| `from` | 今日の 00:00 | 日付として parse できる文字列。`Asia/Tokyo` の日付境界に丸める |
| `to` | `from + rangeDays` | 指定時は `Asia/Tokyo` でその日の翌日 00:00 未満まで |
