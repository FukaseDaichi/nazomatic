...
**ステータス: Phase 4 まで実装済みです。** 検索 prepare API、ローカル候補文（fallback）、validator、ローカルブラウザ投稿 CLI、文案生成 provider、Codex automation 向けの実行設定まで実装済みです。
...
**投稿方針（2026-06-27 改訂、最新）:** このアカウントの魅力は人格の**メンヘラっぽい自虐**にあり、これは消しません。改善するのは2点だけ。(1) 一辺倒の解消＝自虐を減らすのではなく、感情の**“温度”を毎回変えて**散らす（すがり／拗ね／深夜／勘違いの希望→急降下／重い愛／虚無／嫉妬／平静を装って崩れる／情緒の乱高下／開き直り）。(2) わかりにくさの解消＝解読の要るオチをやめ、具体名詞で**1秒で着地**。あわせて、改行（タメ→オチの2ビート）を解禁し、文字数を**無料アカウント上限（日本語 140 字）**に合わせます。イベント名・トレンドは主役ではなく“今日のスイッチ”です。validator の改行・文字数ルール、shape（＝感情の温度）の再定義、fallback 候補の全書き換えは**実装済み**で、`src/server/x-browser-posting/trend-joke-post.ts` と `scripts/x-browser-post-trend-joke.mjs` の両方に同じ変更を反映済みです（fallback プール 25 候補・10 温度すべてを網羅、最大重み付け 132 文字。`tsc --noEmit` と `npm run lint` はクリーン）。
...
固定 URL や集計行は付けず、ネタ投稿だけを出します。**主役はこの人格のメンヘラっぽい自虐**です。イベント名やトレンドはその情緒を動かす“今日のスイッチ”にすぎません。

基本は**タメ→オチの2ビート**。改行でタメとオチを分けてリズムを作ります（長文化のためではない）。短い1行の一撃も混ぜます。
...
生成文は最優先候補として扱います。ただし必ず `validateTrendJokeText()` と直近履歴ガードを通し、URL、hashtag、mention、emoji、重すぎる文字数、近すぎる言い回しは止めます。provider が失敗した場合、生成文が validator に落ちた場合、または履歴ガードで弾かれた場合は、既存のローカル fallback 候補に戻します。文案を固定したい場合は、従来どおり `--line` か `X_BROWSER_POST_TREND_JOKE_LINE` が最優先です。
...
- validator（server と CLI の両方）が URL、hashtag、メンション、絵文字、重み付け 280 超の長文（無料アカウント上限。日本語約 140 字）、不正な改行（`\r`・3 連続以上の改行・行数超過）、断定キーワードを止めることを確認する。`\n` のタメ→オチ（空行1つ）は許可されることを確認する。
