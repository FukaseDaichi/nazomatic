...
2. AI またはテンプレートで短いフォローコメントを生成する。
...
| F-6  | AI コメントは短く、元 Post の内容を過度に複製せず、事実を捏造しない                                 |
| F-7  | AI コメントは投稿前に確認・編集できる。`interactive` では人間確認を必須にする                       |
...
```mermaid
flowchart LR
  operator["人間 / Codex"] --> cli["local CLI"]
...
| comment provider  | `src/server/x-browser-posting/comment.ts`                     | テンプレート、手入力、将来の LLM 連携を抽象化                  |
...
- 毎回 `みなさんぜひ！` だけのような実質同一文面を避ける。

Provider 方針:

| Provider              | 位置づけ                                                                 |
| --------------------- | ------------------------------------------------------------------------ |
| `template`            | MVP。固定テンプレートから候補を出し、人間が選ぶ                          |
| `stdin` / `--comment` | Codex が生成したコメント、または人間が書いたコメントを CLI に渡す        |
| `llm`                 | 将来案。LLM API を使う場合もローカル secret とし、`interactive` では投稿前確認を必須にする |

リポジトリ内に LLM API key を保存する前提にはしません。Codex Desktop から運用する場合は、Codex が候補文を作り、CLI へ `--comment` で渡す形を優先します。
...
- Codex 生成コメントを `--comment` で渡す運用を整える。
- 必要なら `llm` provider を追加する。
- 直近投稿との類似度チェックを強化する。
...
- LLM provider をリポジトリに持つか、Codex が `--comment` を渡す運用に留めるか。
