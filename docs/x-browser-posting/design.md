...
**ステータス: 基本実装済みです。** 実装の正本は `src/` と `scripts/`、既存の X API 再投稿仕様は `docs/calendar-realtime/design.md` を参照します。個別イベント引用投稿では、直近投稿文との類似度チェックはまだコード上のガードとしては実装されていません。トレンドネタ投稿は別途ローカル履歴ガードを持ちます。
...
| S-2 | 既定の投稿上限は 1 回の実行で 1 件、最短 cooldown 120 分、1 日 6 件にする                  |
| S-3 | 設定上の hard limit として、cooldown 3 分未満、1 日 50 件超を禁止する                      |
...
コメントは「観測担当の短い応援・独り言」に限定します。週末サマリ投稿の人格と同じく、ナゾマティックの中にいる AI が、現地に行けない悔しさをごまかしながら、予定表や X を見ている体裁にします。例:

- `私は行けないので、行ける人に届いてほしい。`
- `予定表に余白がある人、そっと見てみてください。`
- `Xとにらめっこ中の観測担当より、届いてほしい一件です。`

通常運用では `src/server/x-browser-posting/comment-patterns.json` に定義した 50 パターンから、prepare のたびにランダムで 1 件を選びます。イベント内容からコメントを生成しないため、チケットの条件やイベント詳細を断定する文面は定義しません。毒は自分自身か観測担当としての立場に向け、参加者、譲渡投稿者、主催者を刺さない文面にします。
...
| `X_BROWSER_POST_HASHTAG` | 任意 | 個別イベント引用投稿の対象 hashtag。既定 `#謎チケ売ります` |
| `X_BROWSER_POST_API_BASE_URL` | 任意 | CLI が呼び出す API origin。未設定時は `REALTIME_API_BASE_URL`、`NEXT_PUBLIC_BASE_URL`、`http://localhost:3000` の順に使う |
| `X_BROWSER_POST_INTERNAL_TOKEN` | 任意 | 内部 API Bearer token。未設定時は `REALTIME_INTERNAL_API_TOKEN` または `REALTIME_API_TOKEN` を使う |
...
| `X_BROWSER_POST_HEADLESS` | 任意 | Playwright 実行を headless にするか。既定 `false` |
| `X_BROWSER_POST_KEEP_OPEN` | 任意 | 実行後にブラウザを開いたままにするか。既定 `false` |
| `X_BROWSER_POST_RESERVED_BY` | 任意 | Firestore lease の `reservedBy` に入れるローカル識別子。未設定時は `user@hostname` |
...
- 未実装。個別イベント引用投稿の直近投稿文との類似度チェックを強化する。
