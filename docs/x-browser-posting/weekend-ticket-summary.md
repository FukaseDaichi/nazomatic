...
この文書は、`#謎チケ売ります` の週末分を土日別に集計し、短い一言と謎チケカレンダー URL を添えて X に毎日投稿する自動化案をまとめます。

**ステータス: 未実装の実装方針です。** 既存のローカルブラウザ投稿自動化は `docs/x-browser-posting/design.md`、Realtime 取得と Firestore 保存は `docs/calendar-realtime/design.md` を正本とします。
...
- ただし、投稿アカウント単位の rate limit は既存の `xBrowserPostingAccounts` と同じ思想で共有し、週末サマリ投稿も X 投稿回数として数えます。

## 必要な構成

| コンポーネント | 配置案 | 役割 |
|---|---|---|
| 集計 service | `src/server/x-browser-posting/weekend-ticket-summary.ts` | 対象週末の土日別対象 Post 数、対象日、サンプル情報を返す |
| 文案 pattern selector | `src/server/x-browser-posting/weekend-ticket-summary-copy-pattern.ts` | 3つの文案パターンからランダムに方向性を決める |
| 文案 provider | 初期はローカル Codex 運用、将来は差し替え可能な provider | 選ばれたパターンに沿って一言候補を生成する |
| 本文 builder | `src/server/x-browser-posting/weekend-ticket-summary-text.ts` | 固定行、土日別件数、AI 一言、URL を結合し、文字数と禁止表現を検査する |
| prepare API | `src/app/api/internal/x/browser-post/weekend-ticket-summary/prepare/route.ts` | 集計、文案パターン選択、文案生成用 prompt 返却を行う |
| local CLI | `scripts/x-browser-post-weekend-ticket-summary.mjs` または既存 CLI の mode 追加 | prepare、本文入力、確認、投稿、ローカル state 更新をまとめる |
...
Firestore query は、`eventTime >= saturdayStart`、`eventTime < mondayStart` を基本にします。対象 hashtag は `hashtags array-contains` または `sourceQuery` で絞り込みます。`#あり` と `#なし` の両表記を対象にするため、既存の `buildHashtagVariants()` と同じ考え方で複数 query を実行し、最後に重複排除します。
...
サマリ投稿では、専用の Firestore 投稿履歴 collection は作りません。二重投稿防止はローカル CLI の Git 管理外 state で軽く扱います。クラウド側に残すのは、必要な場合の account rate limit 更新程度に留めます。
...
| 月曜から金曜の対象 | その週の土曜と日曜 |
| 土曜・日曜の対象 | 翌週の土曜と日曜 |
| ローカル二重投稿防止 | `accountHandle + hashtag + targetWeekendStartDate + runDate` |

同じ日に手動再実行したい場合は、ローカル state の該当キーを削除するか、明示的な `--force-local-duplicate` 相当のフラグを別途設計します。既定では同じ日・同じ対象週末への二重投稿をローカルで止めます。
...
`runDate` と `weekendStartDate` は任意です。未指定なら実行時刻から対象週末を計算します。手動検証や再投稿時だけ `YYYY-MM-DD` を明示できます。
...
prepare API は「土日別の集計値」「ランダムに選ばれた文案パターン」「文案生成に必要な prompt」を返します。投稿結果の Firestore 保存は行いません。AI 呼び出しを API 内で行うか、CLI 側で Codex に依頼するかは provider として分けます。
...
prepare 時に、次の3パターンから1つをランダムに選びます。選ばれた `copyPattern` は prompt とローカル実行ログにだけ残します。
...
- 同じ言い回しが続かないよう、直近の投稿文があれば prompt に含める。
...
- 直近の一言: 「値段下がったら買おうかな、と思った頃にはだいたい誰かの週末になっています。」
...
- `weekend-ticket-summary.ts` に `countWeekendTicketPosts()` を追加する。
- `weekend-ticket-summary-text.ts` に `buildWeekendTicketSummaryPostText()` と `validateWeekendTicketSummaryLine()` を追加する。
- 実行日から対象週末と `weekendLabel` を返す pure function を追加する。
- まずは AI provider を持たず、CLI 引数 `--line` または template で dry-run できる状態にする。

### Phase 2: 内部 API とローカル state

- prepare API を追加する。confirm API と専用投稿履歴 collection は作らない。
- `accountHandle + hashtag + targetWeekendStartDate + runDate` で同じ日の二重投稿をローカル state で防ぐ。
- 既存の account rate limit と競合しないよう、必要な場合だけ既存 account rate state を使う。
- prepare 時に3つの文案パターンからランダム選択する。
- `event_title_commentary` 用に、表示可能イベントの `ticketTitle` を数件サンプルとして返す。

### Phase 3: ローカル CLI

- `scripts/x-browser-post-weekend-ticket-summary.mjs` を追加するか、既存の `scripts/x-browser-post-events.mjs` に `--mode weekend-ticket-summary` を追加する。
- 既存の `openComposer`, `fillComposer`, `assertSubmitReady`, `submitPost`, `verifyLoggedInAccount` を再利用する。
- 既定は dry-run。実投稿は `--execute` と `interactive` 確認を必須にする。
...
- 無人化する場合は provider を差し替え、生成結果を JSON で受け取り、同じ validator を通す。
...
- 初期は `interactive` で数回運用し、X UI、DB 更新、文案品質が安定してから確認省略を検討します。
...
- 土日合計0件でも投稿するか、投稿をスキップするか。
- 文案生成を「Codex に人間が依頼して `--line` に渡す」運用から始めるか、最初から provider 実装にするか。
- 直近投稿文との類似度チェックをローカル state なしでどこまで行うか。
