...
| UI | Tailwind CSS、shadcn/ui、Radix UI、lucide-react、framer-motion、class-variance-authority、clsx、tailwind-merge |
| 3D 表示 | @react-three/fiber、@react-three/drei、three |
| BLANK25 UI 補助 | react-easy-crop、canvas-confetti |
| サーバー処理 | Next.js Route Handler、Firebase Admin SDK、chrono-node |
| 外部連携 | Yahoo!リアルタイム検索、GitHub API、X API、ローカル Playwright / Chrome DevTools Protocol |
...
| ルート | 主な実装 | 役割 |
|---|---|---|
| `/` | `src/app/(main)/page.tsx` | トップページ。`features.json` をカード一覧に使う。背景は `src/components/common/three-hero-background.tsx`(Three.js による歯車・鍵・南京錠の 3D 浮遊アニメーション。ベース色 `#0a0812`、`prefers-reduced-motion` 時は静止背景のみ) |
...
| `POST /api/internal/x/browser-post/weekend-ticket-summary/prepare` | Bearer | `src/app/api/internal/x/browser-post/weekend-ticket-summary/prepare/route.ts` | 週末サマリ投稿用の土日別件数と文案材料を返す |
| `POST /api/internal/x/browser-post/trend-joke/prepare` | Bearer | `src/app/api/internal/x/browser-post/trend-joke/prepare/route.ts` | Yahoo!リアルタイム検索からイベント名材料とネタ投稿文案を返す |
...
JSON-LD は各公開ページから `Article` に `features.json` の配列 index を渡す実装です。`features.json` の順序変更や新規公開ページ追加時は、対象ページの `Article index` も同時に確認します。
...
`scripts/shift-search-threshold.mjs` の `EXTERNAL_THRESHOLD` 以上のレポートは `external` として扱います。現行しきい値は 3,000 行です。`internal` レポートは本文行を `src/generated/shift-search/internal/*.json` に持ち、`external` レポートは本文をアプリに埋め込まず、外部 URL がある場合は一覧から直接遷移し、外部 URL がない場合は詳細ページで raw GitHub Markdown のダウンロード導線を出します。
