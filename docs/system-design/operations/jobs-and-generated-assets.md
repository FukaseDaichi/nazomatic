...
週次改善エージェントの実装コマンドは `npm run x:growth-improve -- --execute` で、実行ログは `logs/x-growth-improve/`、実験状態は `local/x-browser-posting/experiment-ledger.json` に保存します。Codex automation の月曜12:00枠は未登録で、登録されるまでは手動実行だけです。
