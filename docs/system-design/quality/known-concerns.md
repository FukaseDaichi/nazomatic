...
`experimentLedger.mjs` には `resolveExperiment()` がありますが、`package.json` の script や専用 CLI からは呼び出せません。open 実験を `kept` / `reverted` に更新するにはモジュールを直接呼ぶ必要があり、通常運用で open のまま蓄積する可能性があります。
...
### Realtime prune の dry-run は同じ batch を繰り返す

`POST /api/internal/realtime/prune` は dry-run 時に document を削除せず cursor も進めないため、対象があると同じ最大500件を最大20回読み直します。`checked` と `batches` が実対象数より大きくなり、不要な Firestore read も発生します。
