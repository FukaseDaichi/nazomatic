...
投稿頻度、投稿時刻、投稿上限は自動改善の対象外である。頻度増は期待効果より account 制限リスクが大きいため、改善実験は文案品質、投稿型、時間帯の分析に限定する。境界は次の多重防御で担保する。

- `DENY_PATH_PATTERNS` が `config.mjs`、`.env*`、`.github/`、`middleware.ts`、`package.json` / `package-lock.json` を拒否する。
- `LEGACY_FORBIDDEN_TOKENS` が `max_daily`、`min_cooldown`、`--execute`、`confirmation_mode`、`auto_execute` を拒否する。
- 投稿時刻の正本はリポジトリ外の Codex automation にあり、agent から到達できない。

頻度や時刻を変える場合は自動改善ループではなく人間が監視付きで判断し、[`../operations/x-browser-post-schedules.md`](../operations/x-browser-post-schedules.md) を同じ変更で更新する。
