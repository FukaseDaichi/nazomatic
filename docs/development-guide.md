...
| `npm run x:browser-post:observation-log` | 過去7日・向こう7日の謎チケ件数を集計し、週次観測ログを投稿する CLI |
| `npm run x:browser-post:casual-puzzle` | 日曜に6文字のアナグラムを出題し、月曜に前回の答えを投稿する CLI |
...
npm run x:browser-post:observation-log
npm run x:browser-post:observation-log -- --no-image
npm run x:browser-post:casual-puzzle -- --phase question
npm run x:browser-post:casual-puzzle -- --phase answer
...
週次観測ログは `--no-image` で画像生成・文字合成を省略できます。既定では prepare API が過去7日・向こう7日の件数、タイトル sample、観測コメント、calendar URL、画像 prompt を返します。件数は `eventTime` の日程基準で数えた謎チケ情報であり、在庫や販売枚数ではありません。画像と既定コメントは JST の月曜始まりの週でスマホ・ノート・マグカップ・カレンダーの4場面をローテーションします。CLI は `public/img/calendar-ogp.png` を作業ディレクトリへコピーして imagegen の必須参照に指定し、文字なしのキャラクターイラストを生成します。その後、既存の Playwright で1200×675の画像に確定した対象期間・件数・見出しを合成し、最終本文と画像 path を再検証して composer へ入力します。追加の npm 依存は不要です。画像の参照準備・生成・合成は best effort で、失敗時は本文だけを投稿候補にします。

週次観測ログの画像生成子プロセスは、参照画像の確認・生成・保存を含め最大15分待機します。開始時の上限と終了時の経過時間を記録し、タイムアウト時は直前の stderr（空なら stdout）の末尾4000文字を CLI ログに残します。上限超過時は子プロセスを終了して本文だけの投稿へ進みます。

`local/x-browser-posting/observation-log-state.json` と `casual-puzzle-state.json` は atomic write、account 照合、`lastAttempt`、多重 lock を使い、dry-run では投稿・状態・台帳を変更しません。ゆる出題は `buta.dic` の6文字候補から、同じ文字構成の別語がない答えを選び、辞書語にならない順番へ文字をシャッフルします。answer/display の両方へ denylist を適用し、日曜の問題と翌日以降の答えを pending でつなぎます。不適切語を観測した場合は投稿を止め、denylist を更新してから automation を再開します。
...
| NAZOMATIC X 週次観測ログ投稿 | `x:browser-post:observation-log` | `logs/x-browser-post-observation-log/` |
| NAZOMATIC X ゆる出題投稿 | `x:browser-post:casual-puzzle` | `logs/x-browser-post-casual-puzzle/` |
